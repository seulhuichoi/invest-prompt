  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-zinc-400">종목 배분 & 실행 전략</p>
      </div>

      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-5">
        <div className="space-y-3">
          <p className="text-sm font-semibold text-zinc-200">📌 종목 선택 (복수 선택 가능)</p>
          ...
          {selected.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {selected.map((s) => (
                ...
                >
                  {s}
                  <button onClick={() => toggle(s)} className="hover:text-white">
                    ×
                  </button>
                </span>
              ))}
        ...
        <ButtonGroup
          label="투자금액"
          options={['1000만원', '3000만원', '5000만원', '1억원']}
          value={amount}
          onChange={setAmount}
        />
