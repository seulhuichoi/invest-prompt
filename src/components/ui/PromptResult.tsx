    ...
    <div className="space-y-3 mt-4">
      <div className="bg-zinc-800 border border-zinc-700 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-xs text-zinc-400 font-medium">생성된 프롬프트</p>
          <button
            onClick={handleCopy}
      ...
      <div className="space-y-1.5">
        <p className="text-xs text-zinc-500">AI 도구에서 바로 사용하기</p>
        <div className="flex flex-wrap gap-2">
          {AI_TOOLS.map((tool) => (
            <a
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 hover:border-zinc-500 rounded-full text-sm text-zinc-300 transition-colors"
            >
...
export function PromptHistory({ historyKey }: { historyKey: string }): React.JSX.Element {
  ...
  return (
    <div className="mt-6 space-y-2">
      <p className="text-xs text-zinc-500 font-medium">히스토리 (최근 {history.length}개)</p>
