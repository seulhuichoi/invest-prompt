...
**조건부 마크 토큰:** `{변수?값}`은 해당 변수의 현재 값이 `값`과 같으면 `O`, 아니면 공백으로 치환된다(`applyTemplate`). `parseVariables`는 `?` 앞의 베이스 변수명만 입력 변수로 추출하므로, select 변수 하나로 체크박스 여러 개를 마킹할 수 있다(예: '산업 전문가'의 분석 렌즈 — `[{렌즈?기술예측 중심}] …`).

**자동 변수:** `{날짜}`·`{시간}`은 예약된 자동 변수(`AUTO_VARIABLE_NAMES`). 입력칸·종류 설정 없이 `syncVariableSpecs`에서 제외되고 생성 버튼 게이트에도 영향 없다. `TemplateRunner` 생성 시 `autoVariableValues()`로 현재 **KST** 값이 주입된다 — `{날짜}`→"2026년 6월 13일"(앞자리 0 없음), `{시간}`→"KST 12:55"(24시간제, 2자리). 치환은 문자 그대로(조사 변환 없음).
